...
namespace OriBotV3.CoreImplementation.Handlers {
	public class HandlerRandomModSelector : PassiveHandler {
		...
		private Task OnPresenceUpdated(Presence _, Presence np) {
			...
			else if (Context == predContext)
            {
				IEnumerable<Member> moderator = predContext.Server.FindMembersWithRole(predContext.Server.GetRole(420233409789689876));
				moderator = moderator.Where(moderator =>
                {
					return (moderator.Presence.Status == StatusType.Online || moderator.Presence.Status == StatusType.Idle) && !moderator.Roles.Contains(1046554836214235136);
                });

				if (AvailableMods.Count() == 0)
                {
					AvailableMods = moderator;
					return Task.CompletedTask;
                }

				IEnumerable<Member> modsNotStreaming = moderator.Where(moderator => moderator.Presence.Activities.FirstOrDefault(activity => activity.Type != ActivityType.Streaming) != null);
				IEnumerable<Member> modsNotStreamingOrPlaying = modsNotStreaming.Where(moderator => moderator.Presence.Activities.FirstOrDefault(activity => activity.Type != ActivityType.Playing) != null);
