...
namespace OriBotV3.Utility.Music {
	public class MusicController {
		...
		private MusicController(VoiceChannel musicChannel, TextChannel textChannelForMusic) {
			...
			TransmitHelper.ProgressReportAction += data => {
				// 1 = starting to encode
				// 2 = done encoding, starting to write to cache
				// 3 = done saving to cache
				if (data.Equals(1)) {
					StatusReportMessage = MusicTextChannel.SendMessageAsync(Personality.Get("cmd.ori.music.encodingWait")).Result;
				} else if (data.Equals(2)) {
					// Might send too fast when coupled with 3
					// MusicTextChannel.SendMessageAsync(Personality.Get("cmd.ori.music.encodingCache")).Wait();
				} else if (data.Equals(3)) {
					/*if (StatusReportMessage != null) {
						StatusReportMessage.BeginChanges();
						StatusReportMessage.Content = Personality.Get("cmd.ori.music.encodingDone");
						StatusReportMessage.ApplyChanges("Progress final report.");
						StatusReportMessage = null;
					} else {*/
						MusicTextChannel.SendMessageAsync(Personality.Get("cmd.ori.music.encodingDone")).Wait();
					//}
				} else if (data is ValueTuple<int, int> progress) {
					if (StatusReportMessage != null) {
						StatusReportMessage.BeginChanges(true);
						StatusReportMessage.Content = Personality.Get("cmd.ori.music.encodingWait") + $"\n{pbar.GetProgressBarText(progress.Item1 / (double)progress.Item2)}\n({string.Format("{0:n0}", progress.Item1)} of {string.Format("{0:n0}", progress.Item2)} samples processed...)";
						StatusReportMessage.ApplyChanges("Progress report.");
					} else {
						string cnt = Personality.Get("cmd.ori.music.encodingWait") + $"\n{pbar.GetProgressBarText(progress.Item1 / (double)progress.Item2)}\n({string.Format("{0:n0}", progress.Item1)} of {string.Format("{0:n0}", progress.Item2)} samples processed...)"; ;
						StatusReportMessage = MusicTextChannel.SendMessageAsync(cnt).Result;
					}
				} else if (data is ValueTuple<string, int> poggers) {
					if (StatusReportMessage != null) {
						StatusReportMessage.BeginChanges(true);
						StatusReportMessage.Content = Personality.Get("cmd.ori.music.encodingWait") + $"\n{pbar.GetProgressBarText(1)}\n({string.Format("{0:n0}", poggers.Item2)} of {string.Format("{0:n0}", poggers.Item2)} samples processed...)";
						StatusReportMessage.ApplyChanges("Progress report.");
					}
				}
			};
		...
		public Embed GetFormattedNowPlaying(bool justLength = true, bool verboseDir = false) {
			...
			if (NowPlaying != null) {
				...
				VoiceConnectionMarshaller marshaller = VoiceConnectionMarshaller.Current!;
		...
		public async Task PlayAudio(FileInfo file = null) {
			...
			VoiceConnectionMarshaller.Current.RadioTextChannel = MusicTextChannel;
			VoiceConnectionMarshaller.Current.MusicControllerReset = ResetControllerState;
			...
			VoteController.ClearVotes();
			while (Playing) {
				if (VoiceConnectionMarshaller.Current == null) {
					MusicLog.WriteLine("Voice is no longer connected! Terminating.", LogLevel.Trace);
					Playing = false;
					break;
				}
